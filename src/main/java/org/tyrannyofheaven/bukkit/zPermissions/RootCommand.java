...
public class RootCommand {
    ...
    // FIXME entire operation should be done in single transaction
    private void rankChange(ZPermissionsPlugin plugin, CommandSender sender, String playerName, String trackName, boolean rankUp) {
        // Resolve track
        if (!ToHUtils.hasText(trackName))
            trackName = plugin.getDefaultTrack();
        
        List<String> track = plugin.getTrack(trackName);
        ...
        Set<String> trackGroupNames = new HashSet<String>(track);

        Set<String> playerGroupNames = new HashSet<String>();
        plugin.getDatabase().beginTransaction(); // FIXME ugh, has to be better way...
        try {
